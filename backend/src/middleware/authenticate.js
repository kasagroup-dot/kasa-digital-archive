...
export async function authenticate(req, _res, next) {
  try {
    ...
    if (runtime.maintenanceMode && user.role !== 'SUPER_ADMIN') {
      ...
      const maintenanceAllowed = ['/auth/logout'].some((suffix) => originalUrl.includes(suffix));
      if (!maintenanceAllowed) throw new AppError(runtime.maintenanceMessage, { statusCode: 503, code: 'MAINTENANCE_MODE' });
    }
