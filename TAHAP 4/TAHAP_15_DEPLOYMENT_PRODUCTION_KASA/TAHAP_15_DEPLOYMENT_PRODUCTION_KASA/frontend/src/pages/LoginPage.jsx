  ...
  async function submit(event) {
    ...
    try {
      ...
      if (err.code === 'INVALID_CREDENTIALS' || err.status === 401) {
        ...
        setError(err.message || 'Sistem sedang maintenance. Hanya Super Admin yang dapat masuk.');
      } else if (err.code === 'REQUEST_TIMEOUT') {
        setError('Login terlalu lama merespons. Backend atau Supabase sedang tidak merespons; coba sekali lagi.');
    ...
    <main className="login-layout">
      ...
      <section className="login-panel">
        <div className="login-panel__content">
          ...
          {maintenance?.enabled ? (
            <div className="login-maintenance-banner" role="status">
              <strong>MAINTENANCE MODE</strong>
              <span>{maintenance.message || 'Sistem sedang dalam pemeliharaan.'}</span>
              <small>Untuk sementara hanya Super Admin yang dapat masuk.</small>
            </div>
          ) : null}
