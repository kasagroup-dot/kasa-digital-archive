...
import DocumentCollectionPage from './DocumentCollectionPage.jsx';
...
export default function DashboardPage({ user, onLogout }) {
  ...
  function handleNavigate(key, label) {
    ...
    if (key === 'documents') { setActiveView('documents'); return; }
    if (key === 'recent') { setActiveView('recent'); return; }
    if (key === 'favorites') { setActiveView('favorites'); return; }
    if (key === 'recycle') { setActiveView('recycle'); return; }
    if (key === 'activity') { setActiveView('activity'); return; }
    if (key === 'users') { setActiveView('users'); return; }
    if (key === 'permissions') { setActiveView('permissions'); return; }
    if (key === 'settings') { setActiveView('settings'); return; }
    showNotice(`${label} akan dimigrasikan pada tahap berikutnya sesuai urutan migrasi.`);
  }

  function handleSearch(term) {
    if (['divisions','fileManager','documents','recent','favorites','activity','users','permissions'].includes(activeView)) {
      setTopSearch(term || '');
      return;
    }
    if (term) {
    ...
    showNotice('Masukkan kata pencarian terlebih dahulu.');
  }
  ...
  const titleMap = { dashboard:'Dashboard', divisions:'Semua Divisi', documents:'Dokumen', recent:'Dokumen Terbaru', favorites:'Favorit', recycle:'Recycle Bin', activity:'Activity Log', users:'Manajemen User', permissions:'Permission', settings:'System Settings', fileManager:'File Manager' };
  const title = titleMap[activeView] || 'KASA Digital Archive';
  const activeKey = activeView === 'fileManager' ? 'divisions' : activeView;
      ...
      <div className="app-main">
        ...
        {activeView === 'divisions' ? (
          ...
          <DocumentsPage
            ...
            onDivisionSync={(divisionId) => { if (user?.role !== 'SUPER_ADMIN') setSelectedDivision(divisionId); }}
          />
        ) : activeView === 'recent' ? (
          <DocumentCollectionPage mode="recent" selectedDivision={selectedDivision} searchTerm={topSearch} />
        ) : activeView === 'favorites' ? (
          <DocumentCollectionPage mode="favorites" selectedDivision={selectedDivision} searchTerm={topSearch} />
        ) : activeView === 'recycle' ? (
          <RecycleBinPage selectedDivision={selectedDivision} />
        ) : activeView === 'activity' ? (
          <ActivityLogPage selectedDivision={selectedDivision} searchTerm={topSearch} />
        ) : activeView === 'users' ? (
          <UserManagementPage currentUser={user} topSearch={topSearch} />
        ) : activeView === 'permissions' ? (
          <PermissionsPage topSearch={topSearch} />
        ) : activeView === 'settings' ? (
          <SystemSettingsPage />
