...
import {
  ...
  getSystemCounts,
  listAppSettings,
  revokeActiveNonSuperAdminSessions,
  upsertAppSettings
...
export async function updateSystemSettings({ auth, payload = {}, ipAddress, userAgent }) {
  ...
  const beforeRuntime = await getRuntimeSettings({ force: true });
  ...
  // Jangan hanya percaya response upsert. Baca ulang setting dari database dan pastikan
  // Maintenance Mode yang tersimpan sama persis dengan pilihan Super Admin.
  invalidateRuntimeSettings();
  const persistedRuntime = await getRuntimeSettings({ force: true });
  if (persistedRuntime.maintenanceMode !== maintenanceEnabled) {
    throw new AppError('Maintenance Mode gagal dipersist ke database. Silakan ulangi Simpan Settings.', {
      statusCode: 500,
      code: 'MAINTENANCE_PERSIST_FAILED'
    });
  }

  // Saat maintenance baru diaktifkan, cabut session aktif non-Super Admin.
  // Ini membuat mode maintenance berlaku segera, bukan menunggu refresh token kedaluwarsa.
  let revokedSessions = 0;
  if (!beforeRuntime.maintenanceMode && persistedRuntime.maintenanceMode) {
    revokedSessions = await revokeActiveNonSuperAdminSessions('MAINTENANCE_MODE_ENABLED');
  }
  ...
  return { ...saved, enforcement: { maintenanceEnabled: saved.maintenance.enabled, revokedSessions } };
}
