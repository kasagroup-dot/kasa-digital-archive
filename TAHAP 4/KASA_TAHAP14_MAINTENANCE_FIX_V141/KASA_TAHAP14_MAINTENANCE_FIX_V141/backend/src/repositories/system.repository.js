...
export async function revokeActiveNonSuperAdminSessions(reason = 'MAINTENANCE_MODE') {
  const supabase = getSupabaseAdmin();
  ...
  const { data, error } = await supabase
    .from('auth_sessions')
    .update({ status: 'REVOKED', revoked_at: now, revoked_reason: String(reason || 'MAINTENANCE_MODE').slice(0, 200) })
    .eq('status', 'ACTIVE')
    .in('user_id', ids)
    .select('id');
  if (error) throw error;
