...
export default function DashboardPage({ user, onLogout }) {
  ...
  function handleSearch(term) {
    if (activeView === 'divisions' || activeView === 'fileManager' || activeView === 'documents') {
      setTopSearch(term || '');
      return;
    }
    if (term) {
  ...
  const title = activeView === 'dashboard' ? 'Dashboard' : activeView === 'divisions' ? 'Semua Divisi' : activeView === 'documents' ? 'Dokumen' : 'File Manager';
  const activeKey = activeView === 'dashboard' ? 'dashboard' : activeView === 'documents' ? 'documents' : 'divisions';
