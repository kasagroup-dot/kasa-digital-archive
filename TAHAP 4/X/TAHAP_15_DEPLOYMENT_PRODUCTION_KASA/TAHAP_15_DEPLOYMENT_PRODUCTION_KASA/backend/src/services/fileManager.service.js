...
export async function getFolderContents({ auth, divisionId, folderId = '', search = '', fileType = '', category = '', sort = 'newest', page = 1, pageSize = null }) {
  requireView(auth);
  ...
  const safePageSize = Math.min(Math.max(Number(pageSize) || runtime.defaultPageSize, 1), runtime.maxPageSize);
