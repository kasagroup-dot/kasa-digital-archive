...
export default function Topbar({ user, title, scope, divisions, selectedDivision, searchValue = '', onDivisionChange, onMenu, onSearch, onLogout }) {
  const [search, setSearch] = useState('');
  const [userMenu, setUserMenu] = useState(false);
  const initial = String(user?.name || user?.username || 'U').slice(0, 1).toUpperCase();

  useEffect(() => { setSearch(String(searchValue || '')); }, [searchValue]);
