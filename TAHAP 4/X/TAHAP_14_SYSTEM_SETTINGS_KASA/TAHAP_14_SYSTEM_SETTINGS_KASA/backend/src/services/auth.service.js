...
import { getRuntimeSettings } from './runtimeSettings.service.js';
import {
  changePasswordHash,
  findUserById,
  findUserByUsername,
...
async function createSession({ user, rememberMe, ipAddress, userAgent }) {
  ...
  const now = new Date();
  const expiresAt = rememberMe ? addDays(now, runtime.rememberSessionDays) : addHours(now, runtime.sessionDurationHours);
...
export async function login({ username, password, rememberMe, ipAddress, userAgent }) {
  ...
  const runtime = await getRuntimeSettings();
  if (runtime.maintenanceMode && user.role !== 'SUPER_ADMIN') {
    throw new AppError(runtime.maintenanceMessage, { statusCode: 503, code: 'MAINTENANCE_MODE' });
  }
