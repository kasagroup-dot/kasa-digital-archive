...
## Tahap 5 — Supabase Connection

Endpoint tambahan:

- `GET /api/v1/health/supabase` — menguji koneksi nyata ke `app_settings` dan `divisions`.

Backend memprioritaskan `SUPABASE_SECRET_KEY` (`sb_secret_...`). `SUPABASE_SERVICE_ROLE_KEY` hanya fallback legacy.
