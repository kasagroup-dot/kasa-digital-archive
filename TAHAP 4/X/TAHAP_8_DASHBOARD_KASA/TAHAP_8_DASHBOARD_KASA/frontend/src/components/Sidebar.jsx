...
export default function Sidebar({ user, collapsed, mobileOpen, onCloseMobile, onNavigate, onLogout }) {
  const isSuperAdmin = user?.role === 'SUPER_ADMIN';
  const initial = String(user?.name || user?.username || 'U').slice(0, 1).toUpperCase();

  function item([label, icon, key], active = false) {
    return (
      <button key={key} className={`side-item ${active ? 'side-item--active' : ''}`} onClick={() => onNavigate(key, label)} title={collapsed ? label : undefined}>
    ...
    <>
      ...
      <aside className={`app-sidebar ${collapsed ? 'is-collapsed' : ''} ${mobileOpen ? 'is-mobile-open' : ''}`}>
        ...
        <nav className="side-nav">
          {mainItems.map((entry) => item(entry, entry[2] === 'dashboard'))}
          {isSuperAdmin ? (
            <>
              <div className="side-section-label">ADMINISTRATION</div>
              {adminItems.map((entry) => item(entry, false))}
            </>
          ) : null}
        </nav>
