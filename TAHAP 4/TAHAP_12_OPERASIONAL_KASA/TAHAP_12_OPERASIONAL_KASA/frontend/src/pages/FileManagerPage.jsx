    ...
    <main className="workspace-content" onClick={() => menuFolderId && setMenuFolderId('')}>
      ...
      <div className="workspace-heading fm-heading">
        ...
        <div className="fm-heading-actions">
          ...
          {data?.capabilities?.upload ? <button className="fm-upload-file" onClick={() => setUploadOpen(true)}><Icon name="upload" size={16} /> Upload File</button> : null}
      ...
      {loading ? <div className="fm-loading-grid">{Array.from({ length: 8 }).map((_, i) => <div className="fm-card fm-card--loading" key={i} />)}</div> : items.length === 0 ? <div className="fm-empty"><Icon name="folder" size={34} /><strong>Folder ini masih kosong</strong><span>Klik <b>Upload File</b> atau <b>Folder Baru</b> untuk menambah arsip.</span></div> : viewMode === 'grid' ? <div className="fm-grid">{items.map((item) => {
        const row = item.data; const isFolder = item.kind === 'folder';
        return <div key={`${item.kind}-${row.id}`} className="fm-card fm-card-interactive" role="button" tabIndex={0} onDoubleClick={() => isFolder && openFolder(row.id)} onClick={() => isFolder ? openFolder(row.id) : setDocumentModal(row)}>
        ...
        const row = item.data; const isFolder = item.kind === 'folder';
        return <tr key={`${item.kind}-${row.id}`} className={isFolder ? 'is-folder-row' : ''} onClick={() => !isFolder && setDocumentModal(row)}><td><button className="fm-name-button" onClick={(e) => { e.stopPropagation(); if (isFolder) openFolder(row.id); else setDocumentModal(row); }}><span className={`fm-mini-icon ${isFolder ? 'is-folder' : ''}`}>{isFolder ? <Icon name="folder" size={16} /> : badge(row.extension)}</span><span>{isFolder ? row.name : row.originalFilename}</span>{isFolder && row.passwordProtected ? <Icon name="lock" size={13} /> : null}</button></td><td>{isFolder ? 'Folder' : String(row.extension || row.fileType || 'FILE').toUpperCase()}</td><td>{isFolder ? '—' : formatBytes(row.fileSize)}</td><td>{formatDate(isFolder ? row.createdAt : row.uploadedAt)}</td><td>{isFolder ? row.createdBy || '—' : row.uploadedBy || '—'}</td><td>{isFolder && row.passwordProtected ? 'Protected' : 'Active'}</td><td className="folder-table-actions">{isFolder ? <><button className="folder-more" onClick={(e) => showFolderMenu(row, e)}><Icon name="more" size={17} /></button>{renderFolderActions(row)}</> : null}</td></tr>;
      })}</tbody></table></div>}
      ...
      {folderModal}
      {uploadOpen ? <UploadModal divisionId={divisionId} folderId={folderId} onClose={() => setUploadOpen(false)} onUploaded={() => { setToast('Upload selesai.'); load(); }} /> : null}
      {documentModal ? <DocumentActionModal documentId={documentModal.id} initialDocument={documentModal} capabilities={data?.capabilities || {}} onClose={() => setDocumentModal(null)} onChanged={() => load()} /> : null}
    </main>
  );
}
